...
const HELP = {
  ...
  eqSub:{title:'EQ Correctiva (Sustractiva)',color:'#ffa94d',que:'Cuatro filtros peak parametrizables orientados al corte. Bandas centradas en 200 Hz (cuerpo bajo/bombo), 300 Hz (mud clásico), 500 Hz (nasalidad) y 1 kHz (dureza/metalismo). Cada banda tiene frecuencia arrastrable, ganancia y Q ajustables.',sirve:'El compresor no distingue entre resonancia y contenido musical: si hay un pico a 300 Hz, lo aplastará todo junto. Recortar primero hace que el compresor trabaje solo sobre dinámica musical.',uso:'Ganancias siempre negativas (−1 a −6 dB). Q alto (4-8) para cortes quirúrgicos; Q bajo (1-2) para suavizar zonas amplias. Arrastra los nodos directamente en el analizador.',tip:'Si no sabes qué cortar, deja las ganancias a 0 dB. No corrijas lo que no está roto.'},
  ...
  eqAdd:{title:'EQ Tonal (Aditiva)',color:'#4dabf7',que:'Siete filtros de amplio espectro que cubren el mismo mapa que el análisis inicial: Sub shelf (40 Hz), Low shelf (120 Hz), Lo-Mid peak (350 Hz), Mid peak (1 kHz), Hi-Mid peak (3 kHz), High shelf (8 kHz) y Air shelf (16 kHz). Los peaks Lo-Mid, Mid y Hi-Mid son arrastrables en frecuencia.',sirve:'Esta EQ está después del compresor a propósito: ecualizamos la señal ya comprimida. Las 7 bandas corresponden exactamente a las 7 zonas del análisis espectral, lo que permite una corrección directa banda a banda.',uso:'Movimientos pequeños: ±1-3 dB. Air +1.5/+2 dB es casi universal en pop-rock. Lo-Mid −1/−2 dB limpia el barro. Arrastra los nodos directamente en el analizador.',tip:'Prueba Lo-Mid −1.5 dB a 350 Hz para limpiar barro sin tocar el cuerpo grave. Es el ajuste que más diferencia el sonido amateur del profesional.'},
...
function autoSettings(analysis){
  ...
  // ── COMP y SAT — adaptar según dinámica existente ────────────────────
  if(dr<2){
    // Extremadamente limitado (rarísimo): bypass comp, SAT mínima de warmth
    s.comp.enabled=false;
    s.comp.ratio=1.0; s.comp.threshold=0; s.comp.makeup=0;
    s.sat.amount=0.10; // calidez siempre presente, independiente del DR
  } else if(dr<5){
    // Muy comprimido: comp suave activo, SAT reducida pero habilitada
    s.comp.ratio=1.5; s.comp.threshold=-14;
    s.comp.makeup=Math.max(0,Math.min(5,(-14-lufs)*.45));
    s.sat.amount=0.13; // warmth mínimo pero audible
  } else {
    // Normal: todo activo con coeficiente makeup más agresivo para impacto real
    s.comp.makeup=Math.max(0,Math.min(8,(-14-lufs)*.50));
...
function drawVUMeter(){
  ...
  if(!aL||!aR){
    // Sin audio activo: limpiar LUFS-M
    const el=document.getElementById('lufsm-readout');
    if(el){el.style.color='var(--muted)';el.textContent='LUFS-M —';}
  }
  if(aL&&aR){
...
async function doLoad(file){
  ...
  try{
    ...
    processAudio(decoded,state.settings).then(async procBuf=>{
      ...
      // Calibración de Makeup Gain: solo empujar HACIA ARRIBA si está más de 1.5 LU por debajo de -14
      // No reducir si el master ya está en zona razonable (-14 a -9 LUFS)
      if(state.settings.comp.enabled && state.procAnalysis.lufs<-15.5){
        const lufsGap=-14-state.procAnalysis.lufs; // siempre positivo aquí
        const adj=Math.min(4,lufsGap*0.8);
        const newMakeup=parseFloat(Math.max(0,Math.min(10,state.settings.comp.makeup+adj)).toFixed(1));
        if(newMakeup>state.settings.comp.makeup){ // solo si sube
