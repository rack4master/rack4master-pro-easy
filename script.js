...
/* ══════════════════════════════════════
   HELP COLORS (content moved to i18n.js HELP_DATA)
══════════════════════════════════════ */
const HELP_COLORS = {
  spectrum:'#4dabf7', chain:'#cc5de8', realtime:'#69db7c', timeline:'#ffa94d',
  hpf:'#ff6b6b', eqSub:'#ffa94d', sat:'#ffd43b', comp:'#69db7c',
  eqAdd:'#4dabf7', width:'#cc5de8', lim:'#f783ac',
};
...
function showHelp(id){
  const h=getHelp(id); if(!h) return;
  const color=HELP_COLORS[id]||'#4dabf7';
  document.getElementById('modalRoot').innerHTML=`<div class="modal-overlay" id="modalOverlay"><div class="modal" style="--mc:${color}"><div class="modal-header"><div class="modal-color-bar"></div><div class="modal-title">${h.title}</div><button class="modal-close" id="modalClose">✕</button></div><div class="modal-body"><div class="modal-section"><div class="modal-section-title">?</div><div class="modal-section-text">${h.que}</div></div><div class="modal-section"><div class="modal-section-title">→</div><div class="modal-section-text">${h.sirve}</div></div><div class="modal-section"><div class="modal-section-title">⚙</div><div class="modal-section-text">${h.uso}</div></div><div class="modal-tip">💡 ${h.tip}</div></div></div></div>`;
...
══════════════════════════════════════ */
function savePreset(){
  const defaultName = (state.fileName.replace(/\.[^.]+$/,'') || 'master') + '_preset';
  const chosen = window.prompt(t('preset.name.prompt') || 'Preset name:', defaultName);
  if(chosen === null) return;                          // user cancelled
  const safeName = (chosen.trim() || defaultName).replace(/[\\/:*?"<>|]/g,'_');
  const preset={version:'2.1',file:state.fileName,date:new Date().toISOString(),settings:deepClone(state.settings)};
  const blob=new Blob([JSON.stringify(preset,null,2)],{type:'application/json'});
  const url=URL.createObjectURL(blob);
  const a=document.createElement('a');
  a.href=url;
  a.download=safeName.endsWith('.mpreset') ? safeName : safeName+'.mpreset';
  a.click();
